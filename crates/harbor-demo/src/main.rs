...
use harbor_core::seal::{HarborSealService, sign_reserve_bytes};
use seal_sdk_rs::session_key::SessionKey;

const ORIGINAL_PACKAGE_ID: &str =
    "0x8b2429358e9b0f005b69fe8ad3cbd1268ad87f35047a21612e082c64824faf8d";
const LATEST_PACKAGE_ID: &str =
...
async fn main() -> anyhow::Result<()> {
    ...
    let privkey_b64 =
        env::var("HARBOR_SERVICE_PRIVKEY").expect("HARBOR_SERVICE_PRIVKEY must be set");
    ...
    let keypair = if privkey_b64.starts_with("suiprivkey") {
        ...
        let privkey_bytes =
            base64::Engine::decode(&base64::engine::general_purpose::STANDARD, privkey_b64)
    ...
    println!(
        "  bucket_id={} digest={}",
        reserved.bucket_id, reserved.digest
    );
    ...
    let finalized = harbor
        ...
        .await?;
    println!(
        "  seal_policy_id={} state={}",
        finalized.seal_policy_id, finalized.state
    );

    // Step 5: Encrypt payload
    println!("\n[5/10] Encrypt sample.txt with Seal");
    let (id_bytes, ciphertext) = seal
        .encrypt(ORIGINAL_PACKAGE_ID, &finalized.seal_policy_id, plaintext)
        .await?;
    println!(
    ...
    harbor
        .poll_until_completed(&reserved.bucket_id, &upload.data.id, |attempt, state| {
            println!("  attempt {}: state={}", attempt, state)
        })
        .await?;

    // Step 8: Download
    println!("\n[8/10] Download ciphertext");
    let downloaded = harbor
        .download_file(&reserved.bucket_id, &upload.data.id)
        .await?;
    ...
    let policy_initial_shared_version =
        fetch_initial_shared_version(&mut sui_client_for_version, &finalized.seal_policy_id)
            .await?;
    println!(
    ...
    let pkg_addr = sui_sdk_types::Address::from_str(ORIGINAL_PACKAGE_ID)
        .unwrap()
        .into_inner();
    let session_key = SessionKey::new(pkg_addr, 10, &mut signer)
        .await
    ...
    harbor
        .delete_file(&reserved.bucket_id, &upload.data.id)
        .await?;
