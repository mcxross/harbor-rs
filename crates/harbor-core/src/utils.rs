...
pub async fn fetch_initial_shared_version(
    ...
    let object = response
        .object
        .ok_or_else(|| anyhow::anyhow!("Object not found in response"))?;
    let owner = object
        .owner
        .ok_or_else(|| anyhow::anyhow!("No owner in object"))?;

    owner
        .version
        .ok_or_else(|| anyhow::anyhow!("No initial_shared_version found for object"))
}
